...
class VisualizationEngine:
    ...
    def create_visualization(self, data: pd.DataFrame, chart_type: str, 
                          ...
                          **kwargs) -> str:
        """Create visualization and return HTML"""
        try:
            if chart_type == 'bar':
                return self._create_bar_chart(data, title, x_col, y_col, **kwargs)
            elif chart_type == 'line':
                return self._create_line_chart(data, title, x_col, y_col, **kwargs)
            elif chart_type == 'scatter':
                return self._create_scatter_plot(data, title, x_col, y_col, **kwargs)
            elif chart_type == 'histogram':
                return self._create_histogram(data, title, x_col, **kwargs)
            elif chart_type == 'box':
                return self._create_box_plot(data, title, x_col, y_col, **kwargs)
            elif chart_type == 'pie':
                return self._create_pie_chart(data, title, x_col, y_col, **kwargs)
            elif chart_type == 'heatmap':
                return self._create_heatmap(data, title, **kwargs)
            elif chart_type == 'correlation':
                return self._create_correlation_matrix(data, title, **kwargs)
            else:
                return self._create_auto_chart(data, title, x_col, y_col)
    ...
    def _create_bar_chart(self, data: pd.DataFrame, title: str, x_col: str, y_col: str, **kwargs):
        """Create bar chart with color palette and hover percentages"""
        # Check if data has percentage column for enhanced hover info
        if 'Percentage' in data.columns and y_col in ['Count', 'count']:
            # Add color differentiation for multiple categories - use color intensity based on values
            if len(data) > 1:
...
class EnhancedPandasAgent:
    ...
    async def _act_on_reasoning(self, query: str, reasoning: Dict[str, Any]) -> Dict[str, Any]:
        ...
        4. For text analysis, use .str.contains() with na=False
        ...
        chart_type = 'bar'
        ```
        # For One perticluar patient information:
        ```python
        patient_id = 'specific_id'  # Replace with actual patient ID
        df_patient = df_patients[df_patients['patient_id'] == patient_id]
        df_tbi = df_tbi_incidents[df_tbi_incidents['patient_id'] == patient_id]
        df_symptom = df_symptom_logs[df_symptom_logs['patient_about_id'] == patient_id]
        df_worst = df_worst_symptoms[df_worst_symptoms['patient_id'] == patient_id]
        df_therapy = df_therapies[df_therapies['patient_id'] == patient_id]
        df_social = df_social_determinants[df_social_determinants['patient_id'] == patient_id]
        df_reference = df_symptom_reference[df_symptom_reference['patient_id'] == patient_id]

        analysis_result = write code to combine these dataframes as needed
        chart_type = None
        ```
    ...
    async def _observe_and_visualize(self, query: str, action_result: Dict[str, Any]) -> Dict[str, Any]:
        """OBSERVE: Analyze results and create visualizations"""
        ...
        if viz_data is not None and isinstance(viz_data, pd.DataFrame) and not viz_data.empty:
            try:
                ...
                logger.error(f"Error creating visualization: {str(e)}")
                visualization_html = f"<p>Could not create visualization: {str(e)}</p>"
    ...
    async def _format_analysis_result(self, query: str, result: Any) -> str:
        """Format the analysis result for user presentation"""
        if result is None:
        ...
        try:
            if isinstance(result, pd.DataFrame):
                if result.empty:
                    ...
                    summary = result.head(10).to_markdown(index=False)
