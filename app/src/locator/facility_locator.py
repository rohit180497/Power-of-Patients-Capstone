...
class GooglePlacesConnector:
    ...
    def _get_search_types_for_facility(self, facility_type: FacilityType) -> List[str]:
        ...
        type_mappings = {
            ...
            FacilityType.REHABILITATION_CENTER: ["physiotherapist", "gym"],
            FacilityType.NEUROTHERAPY_CLINIC: ["doctor", "health"],
            FacilityType.PHYSICAL_THERAPY: ["physiotherapist"],
            FacilityType.MENTAL_HEALTH: ["doctor", "health"],
            FacilityType.URGENT_CARE: ["doctor", "health"],
            FacilityType.EMERGENCY_ROOM: ["hospital"],
            FacilityType.NEUROLOGY_CLINIC: ["doctor", "health"],
            FacilityType.BRAIN_INJURY_CENTER: ["doctor", "health"],
            FacilityType.SPEECH_THERAPY: ["doctor", "health"],
            FacilityType.OCCUPATIONAL_THERAPY: ["physiotherapist", "health"],
            FacilityType.GENERAL_CLINIC: ["doctor", "health"]
        }
        
        return type_mappings.get(facility_type, ["health"])
    ...
    def _sort_facilities(self, facilities: List[HealthcareFacility], location_coords: Dict) -> List[HealthcareFacility]:
        """Sort facilities by rating and distance"""
        
        def sort_key(facility):
            # Prioritize facilities with ratings, then by rating value
            ...
            # Combined score: rating weight + distance weight
            return (rating_score * 0.7) + (distance_score * 0.3)
    ...
    def _generate_response_text(self, facilities: List[HealthcareFacility], location_query: LocationQuery) -> str:
        ...
        for i, facility in enumerate(facilities[:8], 1):  # Show top 8
            ...
            if facility.rating:
                ...
                facility_info.append(f"   {stars} {facility.rating}/5 ({facility.rating_count or 0} reviews)")
